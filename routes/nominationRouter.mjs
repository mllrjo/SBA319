...
import Nomination from "../models/nominations.mjs"
...
router.delete("/:nominee", async (req, res) => {
  try {
    const deletedNominee = await Nomination.findOneAndDelete(
      { nominee: req.params.nominee },
      req.body,
      { new: true }
    );
    if (!deletedNominee) return res.status(404).json({ error: "Nominee not found" });
